...
class WorkerInterface(object):
    ...
    def __init__(self, conn):
        ...
        try:
            ...
            # first incoming message must be an registration message with qid/wid
            assert "type" in message and message["type"] == "registration"
            assert "qid" in message and "wid" in message
        except Exception, e:
    ...
    def setup(self, setup_data):
        ...
        data = {
            ...
            # hostname to harcode in worker
            # (hostname is part of all cloud api request)
            "hostname": "",
    ...
    def assign(self, job_data):
        """ Sends a job to the worker and yields all updates sent back.
            ...
            Worker starts to execute the job after sending back a "processing"
            update. The type of the last update is "finished".
        """
            ...
            try:
                try:
                    ...
                    if self._pending_hb > 2:
                        logger.debug("pending hbs for {0}: {1}".format(
                            self.endpoint, self._pending_hb)
                        )
...
class Worker(object):
    ...
    def do(self):
        ...
        while True:
            ...
            logger.debug('worker {0} got job {1}'.format(
                self.id, job_data["pk"])
            )
            try:
            ...
            except (WorkerGone, IOError):
                self.q.abandon(job_data)  # FIXME restartable?
                self.qm.deregister_worker(self)
                return
            logger.debug('worker {0} got result of job {1}'.format(
                self.id, job_data["pk"])
            )

            if 'exception' in update:
                logger.debug('worker {0} got exception for job {1}: {2}'.format(
                    self.id, job_data["pk"], update["exception"])
                )
